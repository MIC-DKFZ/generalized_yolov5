...
from utils_karol import save_image
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    # Loggers
    if RANK in [-1, 0]:
        loggers = Loggers(save_dir, weights, opt, hyp, LOGGER, log_media=not opt.disable_media_logging)  # loggers instance
        if loggers.wandb:
    ...
    for epoch in range(start_epoch, epochs):  # epoch ------------------------------------------------------------------
        ...
        for i, (imgs, targets, paths, _) in pbar:  # batch -------------------------------------------------------------
            # save_image(imgs, targets, name=os.path.basename(paths[0]))
...
def parse_opt(known=False):
    ...
    parser.add_argument('--disable_media_logging', action='store_true', default=False, help='Disable the logging of media data')
